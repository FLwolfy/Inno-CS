...
/// <summary>
/// Platform-level ImGui implementation which relies only on Inno.Platform.Graphics interfaces.
/// </summary>
internal sealed class ImGuiNETBackend : IImGuiBackend
{
	    ...
	    ImGuiColorSpaceHandling colorSpaceHandling)
    {
	    ...
	    // Window
	    var dpiScaleVec2 = displaySystem.mainWindow.GetFrameBufferScale();
	    m_dpiScale = MathF.Max(dpiScaleVec2.x, dpiScaleVec2.y);
	    
	    // ImGui Renderer
        m_commandList = graphicsDevice.CreateCommandList();
        m_controller = new ImGuiNETController(displaySystem, graphicsDevice, colorSpaceHandling);
    ...
    public void BeginLayoutImpl(float deltaTime)
    {
	    ...
	    // Virtual Context
	    ImGuiNET.ImGui.SetCurrentContext(m_virtualContextPtrImpl);
	    ImGuiNET.ImGui.GetIO().DisplaySize = new Vector2(m_displaySystem.mainWindow.size.x, m_displaySystem.mainWindow.size.y);
	    ImGuiNET.ImGui.NewFrame();
	    ImGuiNET.ImGui.PushFont(m_fontRegular[ImGuiHost.C_DEFAULT_FONT_SIZE]);

	    // Main Context
	    ImGuiNET.ImGui.SetCurrentContext(m_mainMainContextPtrImpl);
	    m_controller.Update(deltaTime, m_displaySystem.mainWindow.GetPumpedEvents());
	    ImGuiNET.ImGui.PushFont(m_fontRegular[ImGuiHost.C_DEFAULT_FONT_SIZE]);
